...
impl KimiCodeParser {
    ...
    pub fn parse(&self, hostname: &str, include_project: bool) -> io::Result<ParserOutput> {
        ...
        self.parse_current(hostname, include_project, &mut output, &mut buckets)?;
        self.parse_legacy(hostname, include_project, &mut output, &mut buckets)?;
    ...
    fn parse_current(
        ...
        for wire in wires {
            ...
            for line in read_snapshot_lines(&wire)? {
                ...
                let Some(timestamp_ms) = event.get("time").and_then(Value::as_i64) else {
                    continue;
                };
